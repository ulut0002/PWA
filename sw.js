const version = 1;
const cacheName = `ULUT0002_PWA_SW.${version}`;
const dataCacheName = "ULUT0002_DATA";
...
self.addEventListener("activate", (ev) => {
  ev.waitUntil(
    caches.keys().then((keys) => {
      return Promise.all(
        keys
          .filter((key) => key != cacheName && key != dataCacheName)
...
self.addEventListener("fetch", (ev) => {
  ...
  const isChromeExtension = url.protocol
    .toLowerCase()
  ...
  if (isChromeExtension) {
    //don't care if online or offline
